...
Прикладной слой очередей, отдельный от внутренней Redis-очереди LangGraph
Server. Поток такой:

    POST /scrape | /agent/chat/async
        → publish_job() → RabbitMQ → worker → (scrape | run_graph) → set_job()
    GET /jobs/{id}
        → get_job() (читает результат из Redis)

Задача — это словарь ``{"job_id", "type", "payload"}``: ``type`` говорит worker-у,
что делать (``scrape`` или ``chat``), ``payload`` несёт параметры.
"""
