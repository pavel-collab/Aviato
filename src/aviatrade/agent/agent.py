...
Этот модуль оставлен тонким shim'ом, чтобы не ломать существующую точку вызова
в CLI (cli/lib.py:run_agent): она вызывает ``AgentFactory.build_agent(...)`` и
затем ``graph.invoke({"messages": [...]})``.
